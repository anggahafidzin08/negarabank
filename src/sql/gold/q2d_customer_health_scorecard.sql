...
-- Grain: one row per (customer_id, report_month)
--
-- Incremental scope:
--   affected_keys   — (customer_id, report_month) pairs with new Silver data
--   account_monthly — ALL months for affected customers (LAG needs prior months)
--   Final JOIN back to affected_keys ensures we only write changed rows
-- =============================================================================
WITH
...
-- All account months for affected customers — needed so LAG() has prior-month context
account_monthly AS (
  SELECT
    a.customer_id,
  ...
  GROUP BY a.customer_id,
           DATE_TRUNC('month', COALESCE(a.effective_start_date, a.load_date))
),
...
USING (
  SELECT
    ...
    ROUND(
      (mb.total_balance - mb.prev_month_balance)
        / NULLIF(mb.prev_month_balance, 0) * 100, 2
    ...
    ROUND(
      mb.credit_balance / NULLIF(mb.total_credit_limit, 0) * 100, 2
  ...
  FROM mom_balance mb
  -- Re-narrow to affected_keys: LAG above needed the broader customer scope
  JOIN affected_keys ak
    ON  mb.customer_id  = ak.customer_id
    AND mb.report_month = ak.report_month
  LEFT JOIN txn_monthly mt
    ON  mb.customer_id  = mt.customer_id
    AND mb.report_month = mt.report_month
  LEFT JOIN latest_credit_score cs
    ON  mb.customer_id  = cs.customer_id
) AS source
