-- fact_fraud_risk: daily customer risk snapshot, one row per (customer_id, snapshot_date)
WITH daily_txn_metrics AS (
  SELECT
    customer_id,
...
latest_credit_score AS (
  SELECT customer_id, CAST(score AS INT) AS credit_score, probability_of_default
